<p align="center">
<img src="https://i.imgur.com/Clzj7Xs.png" alt="osTicket logo"/>
</p>

<h1>osTicket - Ticket Lifecycle: Intake Through Resolution</h1>
This project will document the lifecycle of an example issue. Please note that, as no actual issues were available, a fabricated issue was used for this demonstration, utilizing resources previously created in an Azure Virtual Machine. As this issue is simulated, the demonstration does not include the resolution. <br />


<h2>Environments and Technologies Used</h2>

- Microsoft Azure (Virtual Machines/Compute)
- Remote Desktop
- Internet Information Services (IIS)

<h2>Operating Systems Used </h2>

- 10 Enterprise multi-session, version 22H2 - x64 Gen2

<h2>Ticket Lifecycle Stages</h2>

- Intake
- Assignment and Communication
- Working the Issue
- Resolution

<h2>The Simulated Issue for "Resolution"</h2>

- From user: Barbie "The entire mobile/online banking system is down."

<h2>Lifecycle Stages</h2>

To begin, from the perspective of our first user, Barbie Doll, I access the user portal from this link, http://localhost/osTicket .
<p>
<img width="842" height="488" alt="Screenshot 2026-08-28 085151" src="https://github.com/user-attachments/assets/54ba1279-2e3c-44b0-8910-164428c750db" />
</p>
<p>
Upon clicking "Open a New Ticket", I fill out the relevant information pertaining to the fabricated issue, and click "Create Ticket".
</p>
<br />

<p>
<img width="843" height="988" alt="Screenshot 2026-08-28 085810" src="https://github.com/user-attachments/assets/9b0d2acf-a488-4879-b1dc-49fc504cf279" />
<img width="838" height="489" alt="Screenshot 2026-08-28 085914" src="https://github.com/user-attachments/assets/5be9ac56-50a3-42e4-9747-60743f0aacfd" />
</p>
<p>
To continue this simulation, I logged in as agent Valerie to observe the ticket. Note that Valerie has "Read only" permissions and is powerless to do anything more than leave notes on the ticket.
</p>
<br />

<p>
<img width="958" height="406" alt="Screenshot 2026-08-28 115034" src="https://github.com/user-attachments/assets/0910c788-9af4-4a87-8a14-2d6ca6752729" />
<img width="960" height="1178" alt="Screenshot 2026-08-28 115546" src="https://github.com/user-attachments/assets/e9809ebe-38f6-454d-8902-fc70327a5929" />
</p>
<p>
I then logged in as the Admin and gave Valerie more permissions. To do so, from the admin panel, I clicked "Agents", and "Valerie Val"
</p>
<br />

<p>
<img width="959" height="398" alt="Screenshot 2026-08-28 115900" src="https://github.com/user-attachments/assets/9d7690f5-5632-4c43-9e43-28405f638f35" />

</p>
<p>
I then clicked "Access" and switched her from "View only" to "Full Access".
</p>
<br />

<p>
<img width="958" height="537" alt="Screenshot 2026-08-28 120355" src="https://github.com/user-attachments/assets/feac9561-d085-4f48-9167-06a73b30d76d" />
<img width="962" height="572" alt="Screenshot 2026-08-28 120414" src="https://github.com/user-attachments/assets/c68af457-72c4-4ce7-98c3-54d71da8c6a1" />
</p>
<p>
Next the Admin, from the agent panel, put a message the ticket, letting Valerie know she now has "Full Access".
</p>
<br />

<p>
<img width="958" height="1264" alt="Screenshot 2026-08-28 120937" src="https://github.com/user-attachments/assets/dc878c95-4afd-42d0-9c93-2349236f4d42" />
</p>
<p>
A quick switch over to Valerie's agent panel shows that now she can properly work the ticket
</p>
<br />

<p>
<img width="960" height="837" alt="Screenshot 2026-08-28 121340" src="https://github.com/user-attachments/assets/75192c7f-21a2-4563-8e78-115244cf61e9" />
</p>
<p>
From Valerie's perspective, she realizes that she is not qualified to handle this ticket. As such she would fix the information within the ticket and escalate it to Theresa, a SysAdmin.
  
  1. The Priority Level should be "Emergency" rather than "Normal". This issue has practically shut the entire company down.

  <img width="647" height="256" alt="Screenshot 2026-08-28 122145" src="https://github.com/user-attachments/assets/9e3143fb-97a4-4a3f-adc6-5e1c5724f735" />

<img width="643" height="252" alt="Screenshot 2026-08-28 122157" src="https://github.com/user-attachments/assets/af37ac64-490a-4b43-bec3-a6e3ec5751d5" />
 
  2. The SLA Plan should be "Sev-A" instead of "Default". This issue needs to be resolved now.

<img width="647" height="249" alt="Screenshot 2026-08-28 122233" src="https://github.com/user-attachments/assets/15f9b74d-a4bf-4ae6-b86f-22045e50db53" />

<img width="642" height="250" alt="Screenshot 2026-08-28 122247" src="https://github.com/user-attachments/assets/94468003-2596-4d64-b67c-ac2bfa7c20e4" />

  3.The help topic should be "Report a Problem/ Business Critical Outage" rather than "General Inquiry/ Other". This issue is too important to be categorized as a "General Inquiry".

<img width="644" height="252" alt="Screenshot 2026-08-28 122311" src="https://github.com/user-attachments/assets/60743267-dc75-45be-983f-47140b7ebbe9" />

<img width="644" height="255" alt="Screenshot 2026-08-28 122324" src="https://github.com/user-attachments/assets/36397ecd-7261-47c0-a6e1-77ed3ce305bc" />
  
  4. This ticket is severe enough that in needs to be assigned to "SysAdmins" for resolution, rather than "Support".

<img width="647" height="270" alt="Screenshot 2026-08-28 122415" src="https://github.com/user-attachments/assets/e78c8ad1-3672-437a-98fe-a4de4f19600b" />

<img width="646" height="276" alt="Screenshot 2026-08-28 122424" src="https://github.com/user-attachments/assets/c3f2009d-adcd-423b-bcf1-580b90d5915b" />

  5. This Ticket needs to be assigned to Theresa as she is a SysAdmin.

<img width="651" height="291" alt="Screenshot 2026-08-28 122500" src="https://github.com/user-attachments/assets/66c368ff-9536-4448-8079-ebe05ca5ffa7" />

<img width="643" height="286" alt="Screenshot 2026-08-28 122511" src="https://github.com/user-attachments/assets/e5dd2fc1-fd58-4c19-91a7-ed4f2681ee75" />

With the corrections completed, the ticked looked like this.

<img width="954" height="532" alt="Screenshot 2026-08-28 122538" src="https://github.com/user-attachments/assets/0a7a8147-4d47-4413-b7a7-4f96869f0dd1" />
</p>
<p>
The next step in this simulation, was to log in as Theresa.
</p>
<br />

<p>
<img width="963" height="365" alt="Screenshot 2026-08-28 125020" src="https://github.com/user-attachments/assets/f9db701b-1a83-49f8-a99c-d2ba66a4c1fa" />
</p>
<p>
Then to have Theresa "work" the ticket to resolution, and close it.
</p>
<br />
<img width="957" height="1232" alt="Screenshot 2026-08-28 125356" src="https://github.com/user-attachments/assets/4b56294c-5072-476b-800e-3c2f5b519c72" />
<img width="958" height="694" alt="Screenshot 2026-08-28 125852" src="https://github.com/user-attachments/assets/43365dab-67f4-4993-9f2b-3deeb1b4f8bb" />
<img width="961" height="1317" alt="Screenshot 2026-08-28 125937" src="https://github.com/user-attachments/assets/e1a1ce15-9690-42be-b288-b7a10422ca64" />

